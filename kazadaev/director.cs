...
namespace kazadaev
{
    public partial class director : Form
    {
        ...
        {
            if (activeEntity == ActiveEntity.Users)
            {
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    {
                        Ispr2525KazadaevNvCursovoiContext context = new();
                        var Users = context.Users.Where(x => x.IdUsers == (int)dataGridView1.SelectedRows[0].Cells[0].Value);
                        try
                        {
                            Users.ExecuteDelete();
                            context.SaveChanges();
            ...
            if (activeEntity == ActiveEntity.Roles)
            {
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    var result = MessageBox.Show("Удалить?", "?", MessageBoxButtons.YesNo);
                    if (result == DialogResult.Yes)
                    {
                        Ispr2525KazadaevNvCursovoiContext contex2 = new();
                        var Roles = contex2.Roles.Where(x => x.IdRole == (int)dataGridView1.SelectedRows[0].Cells[0].Value);
                        try
                        {
                            Roles.ExecuteDelete();
                            contex2.SaveChanges();
        ...
        private void bt_edit_Click(object sender, EventArgs e)
        {
            ...
            if (activeEntity == ActiveEntity.Users)
            {
                try
                {
                    ...
                    {
                        IdUsers = ((int)dataGridView1.SelectedRows[0].Cells[0].Value),
                        UsersName = ((string)dataGridView1.SelectedRows[0].Cells[1].Value),
                        Password = (string)dataGridView1.SelectedRows[0].Cells[2].Value,
                        Role = (string)dataGridView1.SelectedRows[0].Cells[3].Value,
                    };
                    this.Hide();
                    var editing = new editing(ActiveEntity.Users, Users);
            ...
            if (activeEntity == ActiveEntity.Roles)
            {
                try
                {
                    ...
                    {
                        Ispr2525KazadaevNvCursovoiContext context2 = new();
                        dataGridView1.DataSource = context2.Roles.ToList();
                        dataGridView1.Refresh();
                        this.Show();
                    }
                }
        ...
        private void bt_add_Click(object sender, EventArgs e)
        {
            ...
            if (activeEntity == ActiveEntity.Users)
            {
                try
                {
                    var Users = new Models.User
                    {
                        IdUsers = (int)dataGridView1.Rows[dataGridView1.RowCount - 1].Cells[0].Value + 1,
                        UsersName = "",
                        Password = "",
                        Role = "",
                    };
